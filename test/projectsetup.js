...
const handleReactAsync = (done, startTime, waitTime, callbackCheck) => {
  ...
  // Required for enzyme under react 16.
  update()
}

export const handleReactAsyncStart = (done, wTime, callbackCheck) => {
  if (typeof wTime === 'undefined') {
    wTime = waitTime
  }
  intervalID = setInterval(handleReactAsync, 10, done, new Date(), wTime, callbackCheck)
}

export const find = selector =>
  appMount.find(selector)
